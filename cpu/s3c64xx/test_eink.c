...
void epd_send_command(unsigned short command, unsigned short data)
{
    ...
    while(!value)
	{
        value =(__raw_readl(GPODAT))&(0x1<<4);
	}
 ...
 */
 		u16   a = epd_read_epson_reg(0x10, 0x394),
			  b = epd_read_epson_reg(0x10, 0x396),
			  c = epd_read_epson_reg(0x10, 0x398),
              d = epd_read_epson_reg(0x10, 0x39C),
              e = epd_read_epson_reg(0x10, 0x39E);
...
/*  	write_epson_reg(0x0011, 0x310, (iba & 0xFFFF));
    write_epson_reg(0x0011, 0x312, ((iba >> 16) & 0xFFFF));   */
  	write_epson_reg(0x0011, 0x310, 0x68C8);
    write_epson_reg(0x0011, 0x312, 0x18);   
}
...
{		
	...
	char *image = malloc(499200 * sizeof(char));
	char s[100];
	unsigned short value = 0;
 	//display black
    	value = 0x2 << 4;
...
/*  	setenv("copynand", "nand read 55000000 200000 3c000");
	...
	//image = test;
   	//run_command("movi read 3785702 7D000 55000000", 0);
 	sprintf(s, "movi read %d 0x7D000 0x55000000", (ofsinfo.kernel - 512 * 2));
	run_command(s, 0);
	image = (volatile char *)0x55000010;   
	//image = test;
 		epd_send_command(0x0B,(0x2 << 4));	
	
  	//INIT_ROTMODE[0140h] Rotation 90
		epd_send_command(0x0B,(0x3 << 8));	 
			...
			for(j = 415; j > 0; j=j-2)
				{
					if(j == 415);
						...
						__raw_writew((uint32_t)((image[i * 416 + j]| image[i * 416 + j - 1] << 8)), bsaddr);
...
/*   		for(i=0;i<124200;i++)
		{
			__raw_writew((uint32_t)0xFFFF,bsaddr);
		}   */     
 		for(i=0;i<249600;i++)
		{
			__raw_writew(0xFFFF,bsaddr);
...
//now HIRQ  setup to input gpo3
	...
	u16 rd_reg;
    unsigned short value = 0;	
    rd_reg = epd_read_epson_reg(0x10, 0x00a) & ~(1 << 12);
    write_epson_reg(0x0011, 0x00a, (rd_reg | (1 << 12)));		//REG[000Ah] bit 12 = 1b
	mdelay(4);
	
	write_epson_reg(0x0011, 0x010, 0x0003);
    write_epson_reg(0x0011, 0x012, 0x5949);
    write_epson_reg(0x0011, 0x014, 0x0040);
	
	///////////////////////////////////////////////////	
	
	init_controller();
	write_epson_reg(0x0011, 0x204, 0x99);
	bs_cmd_get_wfm_info();
