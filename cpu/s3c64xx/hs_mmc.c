...
{
	printf("the hsmmc_set_base movi_ch is %d ...\n", movi_ch);
...
{
	uint card_size;
	uint blk_nr, sector_nr;
	char s[100];//add by James 2010.9.9
...
/*  		if (movi_hc) {
		...
		//}
			blk_nr = (c_size + 1) * (1 << (c_size_multi + 2));
			sector_nr = blk_nr * blk_size / 512;
			if(movi_ch == 1)
			{
				printf("the sector size is %d ...\n", sector_nr);
				sprintf(s, "%d",  sector_nr);
				setenv("excard_blk", s);
			}
...
void movi_read (uint addr, uint start_blk, uint blknum)
{
	...
	//printf("the dst is 0x%08x ...\n", addr);
	//printf("the blknr is %d ...\n", start_blk);
	//printf("the blkcnt is %d ...\n", blknum);
...
/* 	if (blkcnt != 0)
	...
	return blkcnt; */		
	ulong blk_cnt = blkcnt;
 		while (blkcnt > 0) {
			if (blkcnt < MOVI_RW_MAXBLKS) {
				movi_read((uint) dst, (uint)blknr, (uint)blkcnt);
				blkcnt = 0;
			} else {
				movi_read((uint) dst, (uint)blknr, MOVI_RW_MAXBLKS);
				//printf("***************before the dst is 0x%08x ...\n", dst);
				//printf("*before the movi_rw_maxblks is %d,movi_blksize=%d,multip=%ld ...\n",MOVI_RW_MAXBLKS,MOVI_BLKSIZE,(MOVI_RW_MAXBLKS * MOVI_BLKSIZE) );
				dst += (MOVI_RW_MAXBLKS * MOVI_BLKSIZE / 4);
				//printf("***************after the dst is 0x%08x ...\n", dst);
				blknr += MOVI_RW_MAXBLKS;
				blkcnt -= MOVI_RW_MAXBLKS;
					}
		}
