...
int board_onenand_init (struct onenand_chip *onenand)
{
	...
	onenand->options |= (ONENAND_READ_POLLING | ONENAND_CHECK_BAD /* | ONENAND_PIPELINE_AHEAD */);
	...
	writel(ONENAND_FLASH_AUX_WD_DISABLE, onenand->base + ONENAND_REG_FLASH_AUX_CNTRL);
#endif
...
//	writel(ONENAND_ACC_CLOCK_100_50, onenand->base + ONENAND_REG_ACC_CLOCK);
	//writel(0, onenand->base + ONENAND_REG_ACC_CLOCK);
	...
	/* Enable Interrupts */
	writel(0x3fff, onenand->base + ONENAND_REG_INT_ERR_ACK);
	writel(0x37ff, onenand->base + ONENAND_REG_INT_ERR_MASK);
	...
	value = (ONENAND_MEM_CFG_SYNC_READ | ONENAND_MEM_CFG_BRL_4 | \
			...
			ONENAND_MEM_CFG_WM_SYNC);
	writel(value, onenand->base + ONENAND_REG_MEM_CFG);
	printf("the onenand->base is 0x%08x ...\n", onenand->base);
