...
get_fatent(fsdata *mydata, __u32 entry)
{
	...
	if (bufnum != mydata->fatbufnum) {
		...
		//printf("the startblock is %ld ...\n", startblock);
		//printf("the getsize is %ld ...\n", getsize);
...
get_cluster(fsdata *mydata, __u32 clustnum, __u8 *buffer, unsigned long size)
{
	...
	//printf("startsect is %ld ...\n", startsect);
	//printf("size/FS_BLOCK_SIZE is %ld ...\n", size/FS_BLOCK_SIZE);
...
/* 		if(flag_long_name) {
	...
	//if have kernel update
	if(!strcmp("zimage_update",name)){
...
/* 		if(flag_long_name) {
	...
	if(!strcmp("ramdisk_update",name))
	{
		printf("Found ramdisk, begin  copy it to mmc! ...\n");
		s = getenv("mmcrcmd");
		run_command(s,0);
		offset_tmp = offset;
		flag_ramdisk = 1;
...
/* 		if(flag_long_name) {
	...
	if(!strcmp("system_update",name))
	{
		printf("Found system, begin  copy it to mmc! ...\n");
		s = getenv("mmcscmd");
		run_command(s,0);
		offset_tmp = offset;
		flag_system = 1;
...
/* 		if(flag_long_name) {
	...
	//if have userdata update
	if(!strcmp("data_update",name)){
		printf("Found userdata, begin  copy it to mmc! ...\n");
		//copy file to sdram
  		s = getenv("mmcdcmd");
		run_command(s,0);
...
int file_fat_copy_onenand(void)
{
	...
	if(flag_kernel)
	{
		s = getenv("onenand_clear_kernel_cmd");
		run_command(s,0);
		for(i = 0;i < 10000; i++)
		udelay(1000);
		s = getenv("onenand_load_kernel_cmd");
		run_command(s,0);
		for(i = 0;i < 10000; i++)
		udelay(1000);
		printf("End of copy kernel to onenand...\n");
	}
			
	if(flag_ramdisk)
	{
		s = getenv("onenand_clear_ramdisk_cmd");
		run_command(s,0);
		for(i = 0;i < 10000; i++)
		udelay(1000);
		s = getenv("onenand_load_ramdisk_cmd");
		run_command(s,0);
		for(i = 0;i < 10000; i++)
		udelay(1000);
		printf("End of copy ramdisk to onenand...\n");
	}
	
	if(flag_system)
	{
		s = getenv("onenand_clear_system_cmd");
		run_command(s,0);
		for(i = 0;i < 30000; i++)
		udelay(1000);
		s = getenv("onenand_load_system_cmd");
		run_command(s,0);
		for(i = 0;i < 10000; i++)
		udelay(1000);
		printf("End of copy system to onenand...\n");
	}
		
	if(flag_userdata)
	{
		s = getenv("onenand_clear_data_cmd");
		run_command(s,0);
		for(i = 0;i < 10000; i++)
		udelay(1000);
		s = getenv("onenand_load_data_cmd");
		run_command(s,0);
		for(i = 0;i < 10000; i++)
		udelay(1000);
		printf("End of copy userdata to onenand...\n");
	}
