...
#define ONENAND_CMD_CPBACK          (0x1B)
#define	CONFIG_MTD_ONENAND_VERIFY_WRITE

#if CFG_ONENAND_BBM
static unsigned long bbm[CFG_ONENAND_RSV_BNR+2]={0};
#endif
...
#if CFG_ONENAND_BBM
static unsigned long onenand_dobbm(struct mtd_info *mtd, unsigned long off)
{
	unsigned long ret = off;
	unsigned long off_int=off&(mtd->erasesize-1);
	int i;
	
	if (bbm[CFG_ONENAND_RSV_BNR+1]&&(off-off_int)&&
		(off-off_int<mtd->size-(CFG_ONENAND_RSV_BNR<<((struct onenand_chip *)(mtd->priv))->erase_shift)))
	{
		for (i=0; i<CFG_ONENAND_RSV_BNR; i++)
		{
			if ((bbm[i]==CFG_ONENAND_BBM_TAG)||(bbm[i]==1)||(bbm[i]==2)) continue;
			if (off-off_int==bbm[i])
			{
				ret = mtd->size-((CFG_ONENAND_RSV_BNR-i)<<((struct onenand_chip *)(mtd->priv))->erase_shift)+off_int;
				break;
			}
		}
	}
...
static int onenand_savebbm(struct mtd_info *mtd)
{
	unsigned long i,j,k;
	size_t retlen;
	int ret=-EIO;
	...
	buf = (unsigned long *)malloc(mtd->writesize);
	if (!buf)
	{
		printk(KERN_ERR "[ERR]onenand_savebbm : Failed to malloc memory.\n");
		ret=-ENOMEM;
		goto ExitHere;
	}
...
RunHere:
	bbm[CFG_ONENAND_RSV_BNR] = 0;
	for (i=0; i<CFG_ONENAND_RSV_BNR; i++)
		bbm[CFG_ONENAND_RSV_BNR]+=bbm[i];
	bbm[CFG_ONENAND_RSV_BNR+1]++;
	if (bbm[CFG_ONENAND_RSV_BNR+1]==0)
		bbm[CFG_ONENAND_RSV_BNR+1]++;
	memcpy(buf, bbm, sizeof bbm);

	for (i=0,j=0; i<CFG_ONENAND_RSV_BNR; i++)
	{
		if (bbm[i]==CFG_ONENAND_BBM_TAG)
		{
			ei.addr = mtd->size-((CFG_ONENAND_RSV_BNR-i)<<((struct onenand_chip *)(mtd->priv))->erase_shift);
			ret = mtd->erase(mtd, &ei);
			if (ret||onenand_block_checkbad(mtd, ei.addr)||
				mtd->write(mtd, mtd->size-((CFG_ONENAND_RSV_BNR-i)<<((struct onenand_chip *)(mtd->priv))->erase_shift), 
				mtd->writesize, &retlen, (unsigned char *)buf)||(retlen!=mtd->writesize))
			{
				bbm[i] = 2;
				for (k=0; k<CFG_ONENAND_RSV_BNR; k++)
				if (!bbm[k]) bbm[k] = CFG_ONENAND_BBM_TAG;
				if (k>=CFG_ONENAND_RSV_BNR)
				{
					printk(KERN_ERR "[ERR]onenand_savebbm : Too many bad blocks!!!\n");
					ret = -ENOSPC;
					goto ExitHere;
				}
				goto RunHere;
...
static int onenand_loadbbm(struct mtd_info *mtd)
{
	unsigned long i,j,k;
	size_t retlen;
	int ret=-EIO;
	...
	if (!buf)
	{
		printk(KERN_ERR "[ERR]onenand_loadbbm : Failed to malloc memory.\n");
		goto ExitHere;
	}
	...
	for (i=0,j=0; i<CFG_ONENAND_RSV_BNR; i++)
	{
		if (mtd->read(mtd, mtd->size-((CFG_ONENAND_RSV_BNR-i)<<((struct onenand_chip *)(mtd->priv))->erase_shift), 
			mtd->writesize, &retlen, (unsigned char *)buf)||(retlen!=mtd->writesize))
		{
			printk(KERN_ERR "[ERR]onenand_loadbbm : Failed to read data from flash.\n");
		}
		else
		{
			if (*(buf+i)!=CFG_ONENAND_BBM_TAG) continue;
			*(buf+CFG_ONENAND_RSV_BNR+2) = 0;
			for (k=0; k<CFG_ONENAND_RSV_BNR; k++)
				*(buf+CFG_ONENAND_RSV_BNR+2) += *(buf+k);
			if (*(buf+CFG_ONENAND_RSV_BNR)!=*(buf+CFG_ONENAND_RSV_BNR+2)) continue;
			if ((*(buf+CFG_ONENAND_RSV_BNR+1)>*(bbm+CFG_ONENAND_RSV_BNR+1))||
				((*(buf+CFG_ONENAND_RSV_BNR+1)<0xF)&&(*(bbm+CFG_ONENAND_RSV_BNR+1)>0xFFFFFFF0)))
				memcpy(bbm, buf, sizeof bbm);
			ret = 0;
			j++;
		}
		if (j>=2) break;
	...
	ei.callback = 0;
	ei.len = mtd->erasesize;
	for (i=0,k=0; i<CFG_ONENAND_RSV_BNR; i++)
	{
		if (bbm[i]==2)
		{
			ei.addr = mtd->size-((CFG_ONENAND_RSV_BNR-i)<<((struct onenand_chip *)(mtd->priv))->erase_shift);
			ret = mtd->erase(mtd, &ei);
			if (ret) continue;
			bbm[i]=onenand_block_checkbad(mtd, mtd->size-((CFG_ONENAND_RSV_BNR-i)<<((struct onenand_chip *)(mtd->priv))->erase_shift))?1:0;
			k=1;
		}
	}
	if (k) ret=onenand_savebbm(mtd);
...
static int onenand_apndbbm(struct mtd_info *mtd, unsigned long off)
{
	int i, ret=0;

	off -= (off&(mtd->erasesize-1));
	if (bbm[CFG_ONENAND_RSV_BNR+1]&&off&&(off<mtd->size-(CFG_ONENAND_RSV_BNR<<((struct onenand_chip *)(mtd->priv))->erase_shift)))
	{
		for (i=0; i<CFG_ONENAND_RSV_BNR; i++)
		if (bbm[i]==off) { bbm[i]=2; }
		for (i=0; i<CFG_ONENAND_RSV_BNR; i++)
		if (!bbm[i]) { bbm[i]=off; break; }
		if (i>=CFG_ONENAND_RSV_BNR)
		{
			printk(KERN_ERR "[ERR]onenand_apndbbm : Too many bad blocks!!!\n");
			return -ENOSPC;
		}
		ret = onenand_savebbm(mtd);
	}
	
	return ret;
}

static int onenand_bdump(struct mtd_info *mtd, unsigned long offs, unsigned long offd, unsigned long pgs)
{
	int ret;
	struct onenand_chip *chip = mtd->priv;
	struct erase_info ei;
	...
	if ((!pgs)||(offs==offd)||(pgs>1<<(chip->erase_shift-chip->page_shift))) return 0;
	
	if ((offs>mtd->size)||(offd>mtd->size)||
		unlikely(offs&(mtd->erasesize-1))||
		unlikely(offd&(mtd->erasesize-1)))
	{
		printk(KERN_ERR"[ERR]onenand_bdump : Invalid address!\n");
		return -EINVAL;
	}
	...
	if (ret)
	{
		printk(KERN_ERR"[ERR]onenand_bdump : Failed to erase the block!\n");
		return -EIO;
	}
	...
	cmd_addr = (void __iomem *)chip->command(mtd, ONENAND_CMD_CPBACK, offs);
	chip->write(ONENAND_DATAIN_COPYBACK_SRC, cmd_addr);
	cmd_addr = (void __iomem *)chip->command(mtd, ONENAND_CMD_CPBACK, offd);
	chip->write(ONENAND_DATAIN_COPYBACK_DST|(pgs), cmd_addr);

	/* wait irq */
	onenand_irq_wait_ack(chip, ONENAND_INT_ERR_INT_ACT);
	onenand_irq_wait_ack(chip, ONENAND_INT_ERR_PGM_CMP);

	/* check fail */
	if (onenand_irq_pend(chip, ONENAND_INT_ERR_PGM_FAIL)) {
		printk(KERN_ERR"[ERR]onenand_bdump : ONENAND_INT_ERR_PGM_FAIL!\n");
		onenand_irq_ack(chip, ONENAND_INT_ERR_PGM_FAIL);
		return -EIO;
	}
...
#if CFG_ONENAND_BBM
	return onenand_block_checkbad(mtd, onenand_dobbm(mtd, ofs));
#else
...
/* 	printf("mtd->size is %ld ...\n ", mtd->size);
	...
	stats = mtd->ecc_stats;
 	...
 	while (!ret) {
		...
		/* get start address to read data */
#if CFG_ONENAND_BBM
		cmd_addr = (void __iomem *)(chip->command(mtd, ONENAND_CMD_READ, onenand_dobbm(mtd, from)));
#else
...
#if CFG_ONENAND_BBM
	cmd_addr = (void __iomem *)chip->command(mtd, ONENAND_CMD_READ, onenand_dobbm(mtd, addr));
#else
...
static int onenand_verify_oob(struct mtd_info *mtd, const u_char *buf, loff_t to, size_t len)
{
	...
	//mtd_oob_mode_t	mode = MTD_OOB_PLACE;//Modify by James
	...
	while (read < len) {
		/* get start address to read data */
#if CFG_ONENAND_BBM
		cmd_addr = (void __iomem *)chip->command(mtd, ONENAND_CMD_READ, onenand_dobbm(mtd, to));
#else
...
#if CFG_ONENAND_BBM
RunHere:
		cmd_addr = (void __iomem *)chip->command(mtd, ONENAND_CMD_PROG, onenand_dobbm(mtd, to));
#else
		...
		if (ret) {
#if CFG_ONENAND_BBM
			if (bbm[CFG_ONENAND_RSV_BNR+1]&&(to<mtd->size-(CFG_ONENAND_RSV_BNR<<((struct onenand_chip *)(mtd->priv))->erase_shift)))
			{
				unsigned long old_to=onenand_dobbm(mtd, to);
				
				/* Deselect and wake up anyone waiting on the device */
				onenand_release_device(mtd);

				if ((!onenand_apndbbm(mtd, to))&&
					(!onenand_bdump(mtd, old_to-(to&(mtd->erasesize-1)), 
									onenand_dobbm(mtd, to)-(to&(mtd->erasesize-1)), 
									(to&(mtd->erasesize-1))>>((struct onenand_chip *)(mtd->priv))->page_shift))) {
					/* Grab the lock and see if the device is available */
					onenand_get_device(mtd, FL_WRITING);
					goto RunHere;
				}
...
/* 	for(i = 0; i < 64; i++)
	...
	{
		oobsize = chip->ecclayout->oobavail;
		//printf("11111111111*********************the oobsize is %d ...\n", oobsize);
		}
	else
	{
		oobsize = mtd->oobsize;
		//printf("22222222222*********************the oobsize is %d ...\n", oobsize);
	}
	column = to & (mtd->oobsize - 1);
...
RunHere:
		...
		buf_poi = (u_int *)chip->oob_buf;//data buf
/* 		for(i = 0; i < 8; i++)
		{
			if(i == 0)
			{
...
#if CFG_ONENAND_BBM
static int onenand_lfmt(struct mtd_info *mtd)
{
	int ret, i;
	loff_t off;
	struct erase_info ei;
	...
	if (ret)
	{
		printk(KERN_ERR "[ERR]onenand_lfmt : Failed to erase blocks!!!\n");
		return ret;
	}
	for (off=mtd->size-(CFG_ONENAND_RSV_BNR<<((struct onenand_chip *)(mtd->priv))->erase_shift),i=0; off<mtd->size; off+=mtd->erasesize,i++)
	{
		if (onenand_block_checkbad(mtd, off)) bbm[i] = 1;
	}
	for (off=0,i=0; off<CFG_ONENAND_RSV_BNR; off++)
	{
		if (!bbm[off])
		{
			bbm[off] = CFG_ONENAND_BBM_TAG;
			i++;
		}
		if (i>=2) break;
	}
	for (off=0; off<mtd->size-(CFG_ONENAND_RSV_BNR<<((struct onenand_chip *)(mtd->priv))->erase_shift); off+=mtd->erasesize)
	{
		if (onenand_block_checkbad(mtd, off))
		{
			printk(KERN_ERR "Block @ 0x%.8X is bad.\n", (unsigned int)off);
			for (i=0; i<CFG_ONENAND_RSV_BNR; i++)
			if (!bbm[i]) { bbm[i]=off; break; }
			if (i>=CFG_ONENAND_RSV_BNR)
			{
				printk(KERN_ERR "[ERR]onenand_lfmt : Too many bad blocks!!!\n");
				//ret = -ENOSPC;
			}
		}
	}

	if (onenand_savebbm(mtd))
	{
		printk(KERN_ERR "[ERR]onenand_lfmt : Failed to save bbm data to flash.\n");
	}
...
#if CFG_ONENAND_BBM
RunHere:
		cmd_addr = (void __iomem *)chip->command(mtd, ONENAND_CMD_ERASE, onenand_dobbm(mtd, addr));
#else
...
#if CFG_ONENAND_BBM
		if (bbm[CFG_ONENAND_RSV_BNR+1]&&(addr<mtd->size-(CFG_ONENAND_RSV_BNR<<((struct onenand_chip *)(mtd->priv))->erase_shift)))
		{
			/* Deselect and wake up anyone waiting on the device */
			onenand_release_device(mtd);

			if (onenand_block_isbad(mtd, addr)) {
				if (!onenand_apndbbm(mtd, addr)) {
					/* Grab the lock and see if the device is available */
					onenand_get_device(mtd, FL_ERASING);
					goto RunHere;
				}
...
#if CFG_ONENAND_BBM
	if (onenand_loadbbm(mtd))
	{
		printk("*** Warning - No valid OneNAND BBM data found.\n");
	}
