...
//add by James
int do_clearenv(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	u8 str[MOVI_ENV_BLKCNT];
	int i;
	
	for(i = 0; i < MOVI_ENV_BLKCNT; i++)
	{
		str[i] = 0xFF;
	}
	
	if (0 == INF_REG3_REG || 7 == INF_REG3_REG)
	{
		movi_write((uint)str, ofsinfo.env, MOVI_ENV_BLKCNT);
		printf("Clear the enviroment variable for movinand OK ...Please reset for take effect.\n");
	}
	else if (BOOT_ONENAND == INF_REG3_REG)
	{
		sprintf(str, "onenand erase 60000 20000");
		run_command(str, 0);
		printf("Clear the enviroment variable for onenand OK ...Please reset for take effect. \n");
	}
	else
	{
		printf("Please check the storage device ...\n");
	}
...
//add by James
U_BOOT_CMD(
	clearenv, CFG_MAXARGS, 0,	do_clearenv,
